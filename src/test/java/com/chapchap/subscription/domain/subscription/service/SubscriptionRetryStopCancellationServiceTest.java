...
class SubscriptionRetryStopCancellationServiceTest {
    ...
    void 재시도대기_거래를_중단하고_다음기간과_구독을_취소예정으로_바꾼다() {
        ...
        service.cancel(10L);
        verify(transaction).stopRetry(); verify(order).cancelBeforeStart(); verify(histories).save(any()); assertThat(subscription.getStatus()).isEqualTo(SubscriptionStatus.CANCELLATION_SCHEDULED); assertThat(period.getStatus()).isEqualTo(SubscriptionPeriodStatus.CANCELED_BEFORE_START);
    }
}
