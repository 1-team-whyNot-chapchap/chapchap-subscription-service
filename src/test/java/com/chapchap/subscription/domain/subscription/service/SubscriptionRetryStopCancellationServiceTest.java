...
class SubscriptionRetryStopCancellationServiceTest {
    ...
    void 재시도대기_거래를_중단하고_다음기간과_구독을_취소예정으로_바꾼다() {
        ...
        var paymentPublisher=mock(com.chapchap.subscription.global.kafka.customer.CustomerPaymentEventPublisher.class); var customerPublisher=mock(com.chapchap.subscription.global.kafka.customer.CustomerSubscriptionNotificationPublisher.class);
        SubscriptionRetryStopCancellationService service=new SubscriptionRetryStopCancellationService(subscriptions,periods,payments,orders,histories,time,
            paymentPublisher, customerPublisher);
        ...
        verify(paymentPublisher).publishRetryStoppedAfterCommit(transaction, LocalDateTime.of(2026,9,9,12,0));
        verify(customerPublisher).publishNextPeriodCancellationAfterCommit(subscription, "NOT_REQUIRED", LocalDateTime.of(2026,9,9,12,0));
    }
}
