...
public class SubscriptionPreStartCancellationCompletionService {
    ...
    public void complete(SubscriptionCancellationPreparation prepared) {
        ...
        if (refund.getStatus() != RefundStatus.FINALIZATION_PENDING && refund.getStatus() != RefundStatus.COMPLETED) {
            throw new IllegalStateException("Refund must be ready for finalization first");
        }
        ...
        if (refund.getStatus() == RefundStatus.FINALIZATION_PENDING) {
            ...
            customerRefundPublisher.publishCompletedAfterCommit(
                refund, subscription.getUserId(), refund.getCompletedAt()
            );
        }
